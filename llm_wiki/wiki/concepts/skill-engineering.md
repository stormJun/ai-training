---
type: "concept"
status: "active"
created: "2026-05-03"
updated: "2026-05-03"
aliases:
  - "Skill Engineering"
  - "技能工程"
tags:
  - "ai-training"
  - "llm-wiki"
  - "concept"
  - "concept/skill"
sources:
  - "18_skill_engineering/README.md"
---

# Skill Engineering

## 一句话定义

Skill Engineering 是把可复用任务流程、上下文、约束和操作规范打包成 AI 可以稳定调用的技能单元。

## 为什么重要

LLM Wiki 的维护质量依赖清晰的操作规则。Skill Engineering 提供了写好规则、模板、流程和触发条件的方法，使 AI 不只是回答问题，也能按规范维护知识库。

## 在本仓库中的位置

相关资料位于 `18_skill_engineering/`，入口包括 Skill 访谈问答、Skill authoring best practices 中文材料，以及 OpenAI Skills 最佳实践笔记。

## 相关主题

- [Skill Engineering](../topics/skill-engineering.md)
- [Agent Memory](../topics/memory-patterns.md)

## 相关项目

- 暂无第一批项目页；后续可把 Skill 工程案例整理成项目页或方法页。

## 来源引用

- `18_skill_engineering/README.md`
- `18_skill_engineering/skill_engineering/claude-skills-interview-qa.md`
- `18_skill_engineering/skill_engineering/Skill authoring best practices - Claude API Docs.zh-CN.md`
- `18_skill_engineering/skill_engineering/openai-skills-best-practices.md`

## 维护备注

本页负责沉淀 Skill Engineering 与 LLM Wiki 维护规则之间的关系。

## Obsidian 连接

- [[wiki/index|AI 工程化训练营 LLM Wiki]]
- [[wiki/topics/skill-engineering|Skill Engineering]]
- [[wiki/concepts/schema-guided-ai-maintenance|Schema 驱动的 AI 维护]]
- [[wiki/concepts/llm-wiki|LLM Wiki]]
- [[wiki/concepts/agent-memory|Agent Memory]]
