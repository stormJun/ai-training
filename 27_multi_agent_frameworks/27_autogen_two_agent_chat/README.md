...
1.  **安装依赖:**
    在项目根目录中打开终端，然后运行：
    ```bash
	cd 27_multi_agent_frameworks/27_autogen_two_agent_chat
	pip install uv
    uv sync --locked
    ```
...
2.  **注册内核:**
    ...
    ```bash
    python -m ipykernel install --user --name=multi_agent_protocols --display-name="AI工程化(Multi-Agent)"
    ```
	...
	```bash
	Available kernels:
	multi_agent_protocols     /Users/your_username/Library/Jupyter/kernels/multi_agent_protocols
	python3    /usr/local/share/jupyter/kernels/python3
	```
	如果看到 `multi_agent_protocols` 在列表中，则说明注册成功。
...
3.  **选择内核:**
    可以在 **Kernel > Change kernel** 菜单中看到并选择 **"AI工程化(Multi-Agent)"**。这可以确保您的 notebook 在正确的项目环境中运行。
