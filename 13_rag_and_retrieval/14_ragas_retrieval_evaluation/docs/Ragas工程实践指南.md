...
> 目标读者：已经有 RAG 基础，希望在 `03_rag_and_retrieval/llamaindex_and_ragas` 中快速完成可复现的评估接入。
...
RAGAS 口径（`ragas-main` 与当前 `03_rag_and_retrieval/llamaindex_and_ragas` 环境实现一致）：
...
- 答案正确性 `answer_correctness`（`ragas-main` 与当前 `03_rag_and_retrieval/llamaindex_and_ragas` 环境实现一致）：
...
完整可运行样例：`03_rag_and_retrieval/llamaindex_and_ragas/code/ragas/ragas_full_metrics_demo.py`。
...
本仓库 `03_rag_and_retrieval/llamaindex_and_ragas/code/ragas/ragas_chinese_prompt_tuning.py` 与 `03_rag_and_retrieval/llamaindex_and_ragas/code/ragas/ragas_full_metrics_demo.py` 都已按该方式实现。
...
- 检查 `03_rag_and_retrieval/llamaindex_and_ragas/code/.env` 是否存在且 key 正确
...
处理：在 `03_rag_and_retrieval/llamaindex_and_ragas/code/ragas` 目录运行该脚本，或直接从 `03_rag_and_retrieval/llamaindex_and_ragas` 根目录执行 `python code/ragas/ragas_chinese_prompt_tuning.py`。
...
- 本仓库 RAGAS 代码目录：`03_rag_and_retrieval/llamaindex_and_ragas/code/ragas/`
