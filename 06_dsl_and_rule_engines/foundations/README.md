...
1.  **安装依赖:**
    在项目根目录中打开终端，然后运行：
    ```bash
	cd 06_dsl_and_rule_engines/foundations
	pip install uv
    uv sync --locked
    ```
...
2.  **注册内核:**
    ...
    ```bash
    python -m ipykernel install --user --name=dsl_rule_engines --display-name="AI工程化(DSL)"
    ```
	...
	```bash
	Available kernels:
	dsl_rule_engines     /Users/your_username/Library/Jupyter/kernels/dsl_rule_engines
	python3    /usr/local/share/jupyter/kernels/python3
	```
	如果看到 `dsl_rule_engines` 在列表中，则说明注册成功。
...
3.  **选择内核:**
    可以在 **Kernel > Change kernel** 菜单中看到并选择 **"AI工程化(DSL)"**。这可以确保您的 notebook 在正确的项目环境中运行。
