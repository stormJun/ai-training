...
# 本 notebook 依赖 02_workflows 的依赖版本（LangChain 0.3.x）。
...
# 使用通义千问（需要设置 DASHSCOPE_API_KEY；见 02_workflows/.env）
...
# %%
# 依赖已在 02_workflows/pyproject.toml（uv.lock）里锁定；推荐在终端安装，而不是在 notebook 里 `pip install` 覆盖环境：
# cd 02_workflows && uv sync --locked
