...
- 0. 本仓库学习路线（建议顺序）
- 1. 版本与安装（04_workflow_orchestration/langchain_langgraph_foundations 可复现）
...
1) **Hello World**：先跑通 `StateGraph` + `messages` + `.stream()`
   - Notebook：`04_workflow_orchestration/langchain_langgraph_foundations/p42-langgraph-1.ipynb`
...
4) **Graph API 实战**：重试策略、缓存、分支、循环、Map-Reduce、Command
   ...
   - Notebook：`04_workflow_orchestration/langchain_langgraph_foundations/p47-tool.ipynb`（工具调用循环）
...
6) **快照/回放**：加 checkpointer，理解 `thread_id` / `checkpoint_id`
   - 文档章节：第10章 持久化与快照
   - Notebook：`04_workflow_orchestration/langchain_langgraph_foundations/p44-snapshot.ipynb`

7) **人机协作（HITL）**：暂停/审批/编辑 state 再继续
   - 文档章节：第11章 中断与人机协作
   - Notebook：`04_workflow_orchestration/langchain_langgraph_foundations/HITL.ipynb`
...
9) **记忆管理**：短期记忆 + 长期记忆 + 语义搜索
   - 文档章节：第13章 记忆
   - Notebook：`04_workflow_orchestration/langchain_langgraph_foundations/mem0.ipynb`

**第四阶段：工程化落地**

10) **复杂工作流（RAG 可控循环）**：入口路由 + 评分 + 自纠错循环
    - Notebook：`04_workflow_orchestration/langchain_langgraph_foundations/p42-langgraph-0RAG.ipynb`

11) **调试与观测**：LangSmith 追踪、可视化
    - 文档章节：第14章 调试与观测
    - Notebook：`04_workflow_orchestration/langchain_langgraph_foundations/p45-langgraph-studio.ipynb`

12) **工程化部署**：结构/配置/测试/服务化
    - 文档章节：第15章 工程模板与部署
    - 工程模板：`04_workflow_orchestration/langchain_langgraph_foundations/app`、`04_workflow_orchestration/langchain_langgraph_foundations/app2`（含 `pytest` 示例）
...
![Agent Workflow](/Users/songxijun/workspace/otherProject/ai-training/04_workflow_orchestration/langchain_langgraph_foundations/docs/assets/agent_workflow.png)
...
> `04_workflow_orchestration/langchain_langgraph_foundations/p42-langgraph-0RAG.ipynb` 已按上述方式做了"可选启用"：不配 key 时自动禁用，不影响运行。
...
本仓库在 `04_workflow_orchestration/langchain_langgraph_foundations/` 下有两套可复制模板：

**模板 A：LangGraph Server + Studio（推荐做演示）**

路径：`04_workflow_orchestration/langchain_langgraph_foundations/app`

```bash
cd 04_workflow_orchestration/langchain_langgraph_foundations/app
...
路径：`04_workflow_orchestration/langchain_langgraph_foundations/app2`

```bash
cd 04_workflow_orchestration/langchain_langgraph_foundations/app2
