...
- 事件推送：`SSE`（建议问题异步推送）
- 审计与监控：文件日志 `customer_service_multitenant/logs/requests.log` + 指标快照 `/health`
...
- 日志推送：`customer_service_multitenant/log_push.py` 将 `requests.log` 以 NDJSON 推送至 ELK/Logstash，支持断点续传与鉴权。
