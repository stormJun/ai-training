...
1.  **安装依赖:**
    在项目根目录中打开终端，然后运行：
    ```bash
	cd 04_rag_and_retrieval/02_llamaindex_retrieval_basics
	pip install uv
    uv sync --locked
    ```
...
2.  **注册内核:**
    ...
    ```bash
    python -m ipykernel install --user --name=rag_llamaindex_and_ragas --display-name="AI工程化(RAG)"
    ```
	...
	```bash
	Available kernels:
	rag_llamaindex_and_ragas     /Users/your_username/Library/Jupyter/kernels/rag_llamaindex_and_ragas
	python3    /usr/local/share/jupyter/kernels/python3
	```
	如果看到 `rag_llamaindex_and_ragas` 在列表中，则说明注册成功。
...
3.  **选择内核:**
    可以在 **Kernel > Change kernel** 菜单中看到并选择 **"AI工程化(RAG)"**。这可以确保您的 notebook 在正确的项目环境中运行。
