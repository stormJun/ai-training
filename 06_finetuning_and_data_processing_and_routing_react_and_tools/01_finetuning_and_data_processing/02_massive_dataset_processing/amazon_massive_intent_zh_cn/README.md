...
该目录内的所有文件都来源于 `06_finetuning_and_data_processing_and_routing_react_and_tools/01_finetuning_and_data_processing/02_massive_dataset_processing/chinese_data/zh-CN.jsonl`（MASSIVE 1.1 的 zh-CN 切分），只是根据不同训练场景做了预处理，方便直接喂给分类、指令或对话模型。原始数据的 `partition` 字段被拆分为多个文件，槽位等冗余字段被剥离或改写；若需要完整标注，请回到 `06_finetuning_and_data_processing_and_routing_react_and_tools/01_finetuning_and_data_processing/02_massive_dataset_processing/chinese_data/README.md` 与 `zh-CN.jsonl`。
...
1. **需要槽位/场景？** 返回 `06_finetuning_and_data_processing_and_routing_react_and_tools/01_finetuning_and_data_processing/02_massive_dataset_processing/chinese_data/zh-CN.jsonl`，自行处理所需字段。
