...
`04_local_finetuning_platform` 微调平台需要以下格式：
...
```bash
cd 06_finetuning_and_data_processing_and_routing_react_and_tools/01_finetuning_and_data_processing/02_massive_dataset_processing

# 转换训练集（指令格式）
python convert_massive_to_training_format.py \
  --input amazon_massive_intent_zh_cn/train.jsonl \
  --output amazon_massive_intent_zh_cn/train_converted.jsonl \
  --format instruction \
  --preview

# 转换验证集
python convert_massive_to_training_format.py \
  --input amazon_massive_intent_zh_cn/validation.jsonl \
  --output amazon_massive_intent_zh_cn/validation_converted.jsonl \
  --format instruction

# 转换测试集
python convert_massive_to_training_format.py \
  --input amazon_massive_intent_zh_cn/test.jsonl \
  --output amazon_massive_intent_zh_cn/test_converted.jsonl \
  --format instruction
```
...
```bash
# 查看前 3 条数据
head -n 3 amazon_massive_intent_zh_cn/train_converted.jsonl | python -m json.tool
```
...
```bash
cd 06_finetuning_and_data_processing_and_routing_react_and_tools/01_finetuning_and_data_processing/02_massive_dataset_processing
python ../04_local_finetuning_platform/local_ft/server.py
```
...
# 加载测试集
test_data = []
with open("amazon_massive_intent_zh_cn/test_converted.jsonl") as f:
    for line in f:
