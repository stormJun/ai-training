...
## 代码结构
- `customer_service_multitenant/app.py`：FastAPI 入口与路由（/chat、/health），启动与中间件（请求 ID、日志）。
- `customer_service_multitenant/config.py`：环境变量与默认配置（模型、FAISS 索引目录、DB 路径、检查点类型、人工客服接口 URL）。
- `customer_service_multitenant/statee.py`：LangGraph 状态类型（TypedDict），含 query、intent、kb_answer、order_summary、human_handoff、sources、route。
- `customer_service_multitenant/prompts.py`：意图识别、检索作答、订单说明等提示词。
- `customer_service_multitenant/tools.py`：
...
- `customer_service_multitenant/graph.py`：构建 LangGraph 流程图、节点与条件边，接入检查点与日志。
- `customer_service_multitenant/config.py` 中统一依赖 `DASHSCOPE_API_KEY`、`KB_INDEX_DIR`、`ORDERS_DB_PATH`、`CHECKPOINT_DB_PATH`、`HUMAN_SUPPORT_URL`。
...
- 运行：`uvicorn customer_service_multitenant.app:app --reload`。
- 验证：
