...
1.  **安装依赖:**
    在项目根目录中打开终端，然后运行：
    ```bash
	cd 04_workflow_orchestration/langchain_langgraph_foundations
	pip install uv
    uv sync --locked
    ```
...
3.  **选择内核:**
    可以在 **Kernel > Change kernel** 菜单中看到并选择 **"AI工程化(Workflow)"**。这可以确保您的 notebook 在正确的项目环境中运行。
