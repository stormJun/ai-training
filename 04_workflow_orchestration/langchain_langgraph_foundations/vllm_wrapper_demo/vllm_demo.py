...
def _ensure_workflow_venv():
    here = os.path.dirname(os.path.abspath(__file__))
    week_dir = os.path.abspath(os.path.join(here, os.pardir))
    venv_python = os.path.join(week_dir, ".venv", "bin", "python")
    if os.path.exists(venv_python):
        if os.path.realpath(sys.executable) != os.path.realpath(venv_python):
            os.execv(venv_python, [venv_python] + sys.argv)

_ensure_workflow_venv()
...
DEFAULT_MODEL_NAME = os.getenv("VLLM_MODEL_NAME", "Qwen/Qwen2.5-0.5B-Instruct")
DEFAULT_BASE_URL = os.getenv("VLLM_BASE_URL", "http://localhost:8000")


def _resolve_local_qwen_snapshot() -> str | None:
    """解析本地缓存的 Qwen2.5-0.5B-Instruct 快照目录。"""
    manual_path = os.getenv("LOCAL_QWEN_MODEL_PATH")
    if manual_path and Path(manual_path).exists():
        return manual_path

    repo_dir = Path.home() / ".cache" / "huggingface" / "hub" / "models--Qwen--Qwen2.5-0.5B-Instruct"
    ...
    if snapshot_dir.exists() and model_file.exists():
        return str(snapshot_dir)
    return None


def _generate_with_local_transformers(prompt: str, max_new_tokens: int = 80) -> str | None:
    """在 vLLM 不可用时，使用本地 Transformers 模型直接推理。"""
    model_path = _resolve_local_qwen_snapshot()
    if not model_path:
        return None
...
def demo_basic_usage():
    ...
    try:
        ...
        # 如果没有运行 vLLM 服务，会尝试本地 Transformers 回退
        result = llm.invoke(prompt)
        print(f"回答: {result}")
    except Exception as e:
        print(f"vLLM 连接失败: {e}")
        local_reply = _generate_with_local_transformers(prompt, max_new_tokens=80)
        if local_reply:
            print("本地模型回退成功。")
            print(f"本地回答: {local_reply}")
        else:
            print("未检测到可用本地模型，使用模拟回答。")
...
def main():
    ...
    print(f" 默认服务地址: {DEFAULT_BASE_URL}")
    local_model = _resolve_local_qwen_snapshot()
    if local_model:
        print(f" 检测到本地模型快照: {local_model}")
    else:
        print(" 未检测到本地模型快照，可通过 LOCAL_QWEN_MODEL_PATH 指定。")
    print()
