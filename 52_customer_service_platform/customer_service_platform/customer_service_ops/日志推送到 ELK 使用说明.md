...
## 概述
- 将 `customer_service_ops/logs/requests.log` 的新增内容增量推送到 Logstash HTTP 接口，供 Elasticsearch/Kibana 分析。
...
## 配置
- 文件：`customer_service_ops/log_push_config.json`
- 关键字段：
  - `input_file`：输入日志文件路径。
...
示例：见已生成的 `customer_service_ops/log_push_config.json`。

## 运行
- 手动一次性增量推送：
  - `python customer_service_ops/log_push.py --mode manual --config customer_service_ops/log_push_config.json`
  - 仅预览不推送：`python customer_service_ops/log_push.py --mode manual --dry-run`
  - 从文件开头重新推送：`python customer_service_ops/log_push.py --mode manual --from-start`

- 守护模式持续跟踪推送：
  - `python customer_service_ops/log_push.py --mode daemon --config customer_service_ops/log_push_config.json`
  - 支持轮转检测与增量处理，按批量与时间窗口刷新。

- 健康检查：
  - `python customer_service_ops/log_push.py --mode health`
...
## 日志与状态
- 运行日志：`customer_service_ops/logs/log_push_service.log`。
- 增量状态：`customer_service_ops/state/log_push_state.json`。可删除以重置增量位置。
